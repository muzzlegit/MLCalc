...
export default function useTowers(player, isSelected, level, value) {
  const { towers, fortifications } = useBattleFieldStore(state => state);
  const playerFunctions = usePlayerStoreFunctions();
  const { addTowers, addFortification, setTowers, setFortification, addBuff, removeBuff } =
    playerFunctions;
  ...
  const onAddButtonClick = () => {
    if (isSelected !== "fortification" && towers.length < 2) {
      addTowers(player, {
        ...towersData[`${isSelected}`][`level${level}`],
        type: `${isSelected}`,
        id: nanoid(),
      });
    } else {
      addFortification(
        player,
        { ...towersData[`${isSelected}`][`level${level}`], type: `${isSelected}`, id: nanoid() },
        value,
      );
    }
  };
