...
export default function Modal({toggleModal}) {
  ...
  const onAddButtonClick = () => {
    if (isSelected !== 'fortification') {
      mainAttackerTowers.push({[`${isSelected}`]: towers[`${isSelected}`][`level${level}`]});
    } else {
      mainAttackerFortification.push({[`${isSelected}`]: towers[`${isSelected}`][`level${level}`]});
    }

    console.log(mainAttackerTowers);
    console.log(mainAttackerFortification);
    checkTowersAmount();
  }
  const checkTowersAmount = () => {
    if(isSelected === 'fortification'){
      setIsButtonActive(false);
    } else {
      if(mainAttackerTowers.length >= 2) {
