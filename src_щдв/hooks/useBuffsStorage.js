...
export default function useBuffsStorage( player ) {
  ...
  useEffect(() => {
    ...
    buffs.forEach( buff => {
      ...
      {
        if( buff.unit === "fortification" ) {
            unitsProperties.fortification[ buff.property ] += buff.value;
        };
    ...
    for (const unit in unitsProperties) {
      for (const property in unitsProperties[ unit ]) {
        if( unit === "fortification" ) return;
        setUnitProperty( player, unit, property, unitsProperties[ unit ][property] )
        }
      }
  }, [ buffs, battlefield, setUnitProperty, player, mainAttackerFraction, mainDefenderFraction, mainDefenderApostate, setFortificationProperty ]);
