...
export default function UnitCard({ unitName }) {
  ...
  const { troops } = usePlayerStoreData(player);
  const { amount, onLevelClick, onAmountChange } = useUnit(player, unitName);

  const { level, attack, attackRate, defense, health, healthRate, persecutionRate } = unit;
